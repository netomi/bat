...
class EncodedFieldTest : DexContentTest<EncodedField>() {
    override fun getTestInstances(): Array<EncodedField> {
        return arrayOf(
    ...
    override fun getFactoryMethod(): Function<DexDataInput, EncodedField> {
        return Function { input -> readContent(input, 0) }
    }

    override fun getWriteMethod(data: EncodedField): Consumer<DexDataOutput> {
        return Consumer { output: DexDataOutput? -> data.write(output!!, 0) }
    }
